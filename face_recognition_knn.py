"""
Ini adalah contoh penggunaan algoritma k-nearest-neighbors (KNN) untuk face recognition.

Penggunaan:

1. Siapkan satu set gambar dari orang yang telah diketahui untuk dikenali. Letakkan gambar pada sebuah direktori
    dengan sub-direktori untuk tiap orang.

2. Kemudian panggil fungsi 'train' dengan parameternya.

3. Panggil fungsi 'predict' dan berikan ke model yang telah terlatih untuk mengenali orang-orang dalam gambar yang tidak dikenal.

NOTE: Proyek ini membutuhkan scikit-learn untuk diinstal, Cara install dengan pip (python 3):
...
def predict(X_img_path, knn_clf=None, model_path=None, distance_threshold=0.4):
    ...
    # return the closest distances of a current point to other neighbors
    closest_distances = knn_clf.kneighbors(faces_encodings, n_neighbors=2)
    are_matches = [min(closest_distances[0][i][0], closest_distances[0][i][1])
...
def show_prediction_labels_on_image(count, img_path, predictions):
    ...
    for name, (top, right, bottom, left) in predictions:
        ...
        draw.rectangle(((left, bottom - h - 10), (max(right, left+w+8),
                                                  bottom)), fill=(0, 0, 255), outline=(0, 0, 255))
...
if __name__ == "__main__":
    ...
    for image_file in os.listdir("knn_examples/test"):
        ...
        show_prediction_labels_on_image(count, os.path.join(
            "knn_examples/test", image_file), predictions)
        count = count + 1
